...
data_df = pd.read_csv('Data/FullData/TeenHardbrakingData.csv',usecols=['Event','Driver','Trip','Week'])
event_df = pd.read_csv('Data/Events/StateFarm-HardbrakingTeenEvent.csv')
summary_df = pd.read_csv('Data/KMeans/TeenDisTime.csv',usecols=['Driver','Trip','Distance'])
ndf = pd.DataFrame(columns=['Driver','Trip','Hardbraking','Distance','Frequency'])
...
for key,value in event_dict.items():
    ...
    ndf = ndf.append({'Driver':key[0],'Trip':key[1],'Hardbraking':value[0],'Distance':distance,'Frequency':freq}, ignore_index=True)
ndf = ndf.sort_values(by=['Driver','Trip'])
ndf = ndf.astype({'Driver':int,'Trip':int,'Hardbraking':int,'Distance':float,'Frequency':float})
ndf.to_csv('Data/KMeans/K_means_Hardbraking.csv',index=False)
