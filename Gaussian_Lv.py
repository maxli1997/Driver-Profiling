...
df = pd.read_csv('Setup_2/kmeans_agegroup_'+str(agegroup)+'.csv')
dis_df = pd.read_csv('Data/Lv_KMeans/LvDisTime.csv')
...
labels = ['conservative','medium risk','high risk']
gmm_ = []
...
for i in range(len(group)-1):
    ...
    ax[0].plot(gmm_x[idx], gmm_[i][idx], 'o',c='Orange')
...
result.to_csv('Setup_2/GMM_agegroup_'+str(agegroup)+'.csv',index=False)
#print (result)

print('Predicting......')
df = pd.read_csv('Setup_2/kmeans_pred_agegroup_'+str(agegroup)+'.csv')
drivers = df.Driver.unique()
driver_num = len(drivers)
th1 = gmm_x[idx][-1]
#th2 = 0.20891089
...
result = pd.DataFrame(result_list).transpose()
result.columns = ['Driver','Risk Level','TripsPerWeek','DistancePerTrip']
result.to_csv('Setup_2/GMM_pred_agegroup_'+str(agegroup)+'.csv',index=False)